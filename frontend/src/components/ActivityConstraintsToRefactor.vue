    ...
    <div>
      ...
      <form @submit.prevent="createActivity">
        ...
        <div v-for="(constraint, index) in constraintsConfig" :key="index">
          ...
          <select v-model="constraint.constraintName" @change="onConstraintDropdownChange(index)">
  ...
  export default {
    ...
    data() {
      return {
        ...
        //constraintsConfig: [new ConstraintConfig('Location', "Outside")],
    ...
    methods: {
      drawExistingConstraintsConfig(){
        this.constraintsConfig.forEach((constraintConfig, index) => {
          const selectedConstraint = this.constraintsDropdownOptions.find(constraint => constraint.name === constraintConfig.constraintName);
          ...
          this.drawAndsetConfig(valuesContainer, selectedConstraint, selectedOperator, constraintConfig);
        })
        console.log(this.constraintsConfig);
        //this.constraintsConfig = this.constraintsConfig.slice();
      },
      addConstraintConfig() {
      ...
      onConstraintDropdownChange(index){
        ...
        const selectedConstraint = this.constraintsDropdownOptions.find(constraint => constraint.name === this.constraintsConfig[index].selectedConstraint);
      ...
      drawAndsetConfig(container, constraint, operator, constraintConfig){
        var userConstraintConfigInput = null;
          if (constraint.values){
              const valuesJSON = JSON.parse(constraint.values);
              userConstraintConfigInput = operator.drawAndsetConfig(container, valuesJSON, constraintConfig.constraintConfig);
          }
          else{
              userConstraintConfigInput = operator.drawAndsetConfig(container, constraintConfig.constraintConfig);
          }
          return userConstraintConfigInput
      },
    },
    mounted() {
      this.fetchAllConstraints()
        .then(() => {
          return this.drawExistingConstraintsConfig();
        })
    },
  };
  </script>
